...
public class BECrusher extends AUBlockEntity {
    ...
    private RecipeHolder<CrusherRecipe> currentRecipe;
    private int progress;

    public BECrusher(BlockPos pos, BlockState state) {
        super(AUBlockEntityTypes.CRUSHER.get(), pos, state);
    }
    ...
    public static void registerCapabilities(RegisterCapabilitiesEvent event) {
        event.registerBlockEntity(
                ...
                AUBlockEntityTypes.CRUSHER.get(),
                (be, ctx) -> be.inventory);
    ...
    protected boolean serverTick(ServerLevel level, BlockPos pos, BlockState state) {
        ...
        if (isLit != canWork()) {
            level.setBlock(pos, state.setValue(BlockCrusher.LIT, canWork()), 3);
        }
    ...
    public @Nullable AbstractContainerMenu createMenu(int id, Inventory playerInv, Player player) {
        return new MenuCrusher(id, playerInv, this);
    }

    public class MenuCrusher extends AUBEMenu {

        public MenuCrusher(int id, Inventory playerInv, AUBlockEntity be) {
            super(AUMenus.CRUSHER.get(), id, be);
            ...
            progressWidget.jeiCategory(AuxiliaUtilities.id(CrusherRecipe.Type.ID));
            ...
            addWidget(new SlotItemHandlerWidget(inventory,2, 122, 42){
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false;
                }
